...
CREATE TABLE `communitygroupsupplier` (
  ...
  UNIQUE KEY `UQ_CommunityGroupSupplier` (`SupplierID`,`CommunityID`,`CommunityGroupID`),
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `ACCOUNTACTIVATE`(
...
-- ---------------------------------------------------------
    ...
    DECLARE v_CommunityOwnerIncome DECIMAL;
    ...
    -- If a value was supplied for the @ActionAmt input parameter        
            ...
            BEGIN
                ...
                INSERT INTO SupplierTransactions(SupplierActionID,CustomerID,SupplierID,CommunityID,CommunityGroupID,TransactionAmount,CurrencyID)
                    VALUES(LAST_INSERT_ID(),IN_CustomerID,IN_SupplierID,IN_CommunityID,IN_CommunityGroupID,IN_ActionAmt,v_Currency);
        ...
        -- If the given Supplier is not currently within a Bill Free Override
           ...
           ELSEIF(EXISTS(SELECT IsActive FROM SupplierCommunityBillFreeOverride WHERE CommunityID=IN_CommunityID AND CommunityGroupID=IN_CommunityGroupID
            ...
            -- @CommunityOwnerIncome (CommunityGroupBillingFee.Fee * TriggeredEvent.BillingPercentageOwner)
              SET v_CommunityOwnerIncome=(SELECT CGB.Fee * TE.BillingPercentageOwner FROM CommunityGroupBilling CGB 
                INNER JOIN TriggeredEvent TE ON CGB.TriggeredEventID=TE.TiggeredEventID WHERE CGB.CommunityGroupID=IN_CommunityGroupID AND TE.ActionID=IN_ActionID);
            
              CALL CreditVirtualCommunityAccount(IN_CommunityOwnerID,'Community Owner',IN_CommunityID,IN_CommunityGroupID,'',v_CommunityOwnerIncome,NOW(),IN_CustomerID);
              SET v_CommunityOwnerTransactionHistoryID = LAST_INSERT_ID();
              
              CALL DebitVirtualCommunityAccount(IN_SupplierID,'Supplier',IN_CommunityID,IN_CommunityGroupID,'',IN_ActionAmt,NOW(),IN_CustomerID);
              SET v_SupplierTransactionHistoryID = LAST_INSERT_ID();
              
              INSERT INTO BillingReference(CommunityOwnerTransactionHistoryID,SupplierCommunityTransactionHistoryID)
                VALUES(v_CommunityOwnerTransactionHistoryID,v_SupplierTransactionHistoryID);
                
               IF(EXISTS(SELECT v_Points=Points FROM CommunityReward WHERE CommunityID=IN_CommunityID)) THEN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `ActionList`()
BEGIN
-- ------------------------------Comment----------------
-- Get all Action
-- -----------------------------------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CHECKCOMMUNITYNAMEEXIST`(
IN IN_CommunityID INT,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CHECKOWNEREMAILEXIST`(
IN IN_OwnerID INT,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CHECKSUPPLIERCOMPANYNAMEEXIST`(
IN IN_SupplierID INT,
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CHECKSUPPLIEREMAILEXIST`(
IN IN_SupplierID INT,
IN IN_Email VARCHAR(150)
)
BEGIN
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `COMMUNITYGROUPSUMMARY`(
IN IN_CommunityGroupID INT
)
BEGIN
-- --------------Comment---------------------------------------
...
-- ------------------------------------------------------------
         ...
         (
            CommunityGroupID INT,CommunityGroupName VARCHAR(150)
            ,InCreditCount INT
            ,OutCreditCount INT,BelowMinCreditCount INT
            ,CustomerCount INT, CurrentRevenue DECIMAL(10,2)
         );
         
         INSERT INTO tmp(CommunityGroupID,CommunityGroupName)
         SELECT SQL_NO_CACHE CommunityGroupID,Name
         FROM  CommunityGroup  
         ...
         CREATE TEMPORARY TABLE tmp_tc as (
             ...
             (SELECT  COALESCE(Balance,0) FROM SupplierCommunityTransactionHistory WHERE cgs.SupplierID=SupplierID 
             AND CommunityGroupID=cgs.CommunityGroupID AND DateApplied= 
             (SELECT  MAX(DateApplied) FROM SupplierCommunityTransactionHistory WHERE cgs.SupplierID=SupplierID AND CommunityGroupID=cgs.CommunityGroupID 
              GROUP BY DateApplied)) AS Balance
              
             FROM CommunityGroup cg JOIN CommunityGroupSupplier cgs ON cg.CommunityGroupID=cgs.CommunityGroupID WHERE cgs.CommunityGroupID =IN_CommunityGroupID
         );
         ...
         UPDATE tmp t 
         SET CurrentRevenue=(SELECT SUM(Balance) FROM CommunityOwnerTransactionHistory WHERE CommunityGroupID=t.CommunityGroupID GROUP BY CommunityGroupID);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `COMMUNITYGROUPSUMMARYBYCOMMUNITY`(
IN IN_CommunityID INT
)
BEGIN
-- -----------Comment-----------------------------------------------------
...
-- -----------------------------------------------------------------------
         ...
         CREATE TEMPORARY TABLE tmp_tc as (
             ...
             FROM CommunityGroup cg JOIN CommunityGroupSupplier cgs ON cg.CommunityGroupID=cgs.CommunityGroupID WHERE cgs.CommunityID =IN_CommunityID
         );
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `COMMUNITYLISTBYSUPPLIER`(
IN IN_SupplierID INT
)
BEGIN
-- ----------------Comment---------------------------
...
-- --------------------------------------------------
    ...
    SELECT Name,cs.CommunityID,
        ...
        (SELECT COUNT(CommunityID) AS Active FROM CommunityGroupSupplier   WHERE cs.CommunityID=CommunityID AND SupplierID = cs.SupplierID) AS Total
    FROM CommunitySupplier cs INNER JOIN Community c ON c.CommunityID=cs.CommunityID WHERE SupplierID=IN_SupplierID;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `COMMUNITYSUMMARY`(
IN IN_CommunityID INT
)
BEGIN
-- -------------------Comment------------------------------
...
-- --------------------------------------------------------
         ...
         CREATE TEMPORARY TABLE tmp
         (
            CommunityID INT,CommunityName VARCHAR(150)
            ,CommunityGroupCount INT,InCreditCount INT
            ,OutCreditCount INT,BelowMinCreditCount INT
            ,CustomerCount INT, CurrentRevenue DECIMAL(10,2)
         );
         
         INSERT INTO tmp(CommunityID,CommunityName,CommunityGroupCount)
         SELECT SQL_NO_CACHE c.CommunityID,c.Name,COUNT(cg.CommunityID) AS CommunityGroups 
         FROM Community c INNER JOIN CommunityGroup cg on c.CommunityID=cg.CommunityID 
         WHERE C.CommunityID=IN_CommunityID GROUP BY c.CommunityID;
         
         CREATE TEMPORARY TABLE tmp_tc as (
             SELECT SQL_NO_CACHE cs.CommunityID,cs.SupplierID,cs.MinCredit, 
             (SELECT  COALESCE(Balance,0) FROM SupplierCommunityTransactionHistory WHERE cs.SupplierID=SupplierID AND CommunityID=cs.CommunityID AND DateApplied= 
             (SELECT  MAX(DateApplied) FROM SupplierCommunityTransactionHistory WHERE cs.SupplierID=SupplierID AND CommunityID=cs.CommunityID 
              GROUP BY DateApplied)) AS Balance
              
             FROM  CommunitySupplier cs WHERE cs.CommunityID =IN_CommunityID
         );

         
         UPDATE tmp t  
         SET t.InCreditCount=(SELECT Count(SupplierID) FROM tmp_tc WHERE Balance>MinCredit AND Balance!=0 AND CommunityID=t.CommunityID);
         
         UPDATE tmp t 
         SET t.BelowMinCreditCount=(SELECT Count(SupplierID) FROM tmp_tc WHERE Balance<MinCredit AND Balance!=0 AND CommunityID=t.CommunityID) ;
         
         UPDATE tmp t  
         SET t.OutCreditCount=(SELECT Count(SupplierID) FROM tmp_tc WHERE Balance=0 AND CommunityID=t.CommunityID) ;
         
         UPDATE tmp t 
         SET CustomerCount=(SELECT COUNT(CustomerID) FROM CustomerCommunity WHERE CommunityID=t.CommunityID);
         
         UPDATE tmp t 
         SET CurrentRevenue=(SELECT SUM(Balance) FROM CommunityOwnerTransactionHistory WHERE CommunityID=t.CommunityID GROUP BY CommunityID);
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `COMMUNITYSUMMARYBYOWNER`(
IN IN_OwnerID INT
)
BEGIN
-- ----------------Comment----------------------------------------
...
-- ---------------------------------------------------------------
         ...
         INSERT INTO tmp(CommunityID,CommunityName,CommunityGroupCount)
         SELECT SQL_NO_CACHE c.CommunityID,c.Name,COUNT(cg.CommunityID) AS CommunityGroups FROM Community c LEFT JOIN CommunityGroup cg on c.CommunityID=cg.CommunityID 
         WHERE OwnerID=IN_OwnerID GROUP BY c.CommunityID;
         
         CREATE TEMPORARY TABLE tmp_tc as (
             SELECT SQL_NO_CACHE c.CommunityID,cs.SupplierID,cs.MinCredit, 
             (SELECT  COALESCE(Balance,0) FROM SupplierCommunityTransactionHistory WHERE cs.SupplierID=SupplierID AND CommunityID=cs.CommunityID AND DateApplied= 
             (SELECT  MAX(DateApplied) FROM SupplierCommunityTransactionHistory WHERE cs.SupplierID=SupplierID AND CommunityID=cs.CommunityID 
              GROUP BY DateApplied)) AS Balance
              
             FROM Community c INNER JOIN CommunitySupplier cs ON c.CommunityID=cs.CommunityID WHERE OwnerID=IN_OwnerID
         );
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`localhost`*/ /*!50003 PROCEDURE `COUNTRYSELECTALL`()
BEGIN
-- -----------------Comment------------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CURRENCYSELECTACTIVE`()
BEGIN
-- ------------Comment-----------------------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CURRENCYSELECTALL`()
BEGIN
-- ------------------Comment-----------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `CUSTOMERSEARCHBYSUPPLIER`(
IN IN_Handle VARCHAR(50),
IN IN_FirstName VARCHAR(50),
IN IN_LastName VARCHAR(50),
IN IN_Email VARCHAR(100),
...
-- ------------------------------------------------------------------
        ...
        WHERE sa1.ActionDate IS NULL AND sa.SupplierID=IN_SupplierID AND (Cu.FirstName LIKE CONCAT('%',IN_FirstName,'%') OR Cu.LastName LIKE CONCAT('%',IN_LastName,'%')
                OR Cu.Handle LIKE CONCAT('%',IN_Handle,'%')) AND (sa.ActionID=IN_ActionID OR IN_ActionID=0)
                AND (sa.CommunityGroupID=IN_CommunityGroupID OR IN_CommunityGroupID=0)
         ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `OWNERSELECT`(
IN IN_OwnerID INT
)
BEGIN
-- -------------------Comment---------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `PreferredShortlistUpdate`(
IN IN_CustomerID INT,
IN IN_SupplierID INT,
IN IN_CommunityID INT,
IN IN_CommunityGroupId INT
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `SocialMediaSelect`()
BEGIN
-- ---------------Comment---------------------------------------------
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `SUPPLIERACTIONSELECTBYCUSTOMER`(
IN IN_CustomerID INT,
IN IN_SupplierID INT
...
-- -----------------------------------------------------------------
    ...
    WHERE cu.CustomerID=IN_CustomerID AND sa.SupplierID=IN_SupplierID ORDER BY sa.ActionDate DESC;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `SupplierCommunityCommunityGroupList`(
IN IN_SupplierID INT
)
BEGIN
-- ---------------Comment---------------------------------------------
...
-- -------------------------------------------------------------------
    ...
    FROM CommunitySupplier cs INNER JOIN  Community c ON cs.CommunityID AND c.CommunityID
    INNER JOIN CommunityGroup cg ON c.CommunityID=cg.CommunityGroupID
    WHERE cs.SupplierID=IN_SupplierID AND cs.IsActive=TRUE 
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `SupplierCommunityMembershipCount`(
IN IN_SupplierID INT
)
BEGIN
-- ------------Comment------------------------------------
...
-- -------------------------------------------------------
    ...
    FROM (SELECT COUNT(CommunitySupplierID) AS Active FROM CommunitySupplier WHERE SupplierID = IN_SupplierID AND IsActive = 1) Active
    CROSS JOIN (SELECT COUNT(CommunitySupplierID) AS Inactive FROM CommunitySupplier WHERE SupplierID = IN_SupplierID AND IsActive = 0) Inactive;   
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `SupplierSocialReferenceSelect`(
IN IN_SupplierID INT
)
BEGIN
-- ------------Comment------------------------------------
...
BEGIN
      ...
      /*IF(EXISTS(SELECT OAuthAccountID From oauthaccount Where Provider=OAuthProvider AND ProviderUSerID=OAuthUserID AND Token=OAuthToken)) THEN
       ...
       IF(EXISTS(SELECT OAuthAccountID From oauthaccount Where Provider=OAuthProvider AND ProviderUSerID=OAuthUserID AND Token=OAuthToken)) THEN           
           SELECT 'valid' as result, a.OAuthAccountID, a.Active, ea.EntityType, ea.EntityID FROM OAuthAccount a 
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`%`*/ /*!50003 PROCEDURE `VALIDATEAPITOKEN`(
IN IN_APIToken VARCHAR(50)
)
BEGIN
-- -------Comment-----------------------------------------
...
-- Dump completed on 2014-04-30 18:43:24
